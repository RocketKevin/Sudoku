...
let valueBoard = []; //1D array representing Sudoku game board
 ...
 * @param {*} valueBoard, board is a 1D array representing Sudoku board
 */
function isRowComplete(rowToVerify) {
  //using const because we don't want to change rowSet's memory address
...
class Board extends React.Component {
  ...
  /**
   * This method returns an array with the location (indicies) of empty squares
   * (not used, but was originally going to use for solver)
   * @param {*} valueBoard, 1D array representing gameboard
   */
  getEmptySquares(valueBoard) {
   ...
   *
   * 7. if there's still empty squares, but no more possible values,
   * return false
   * 
   * @returns true if there's still more empty squares in solution & still more
   * possible values, false if impossinle to solve
   * 
   * @param {*} index 
   * @param {*} valueBoard, 1D array representing Sudoku gameboard
   */
  solveHelper(valueBoard) {
    let row = -1;
    let col = -1;
    let finishProcessing = true;
    ...
    for (let rowIndex = 0; rowIndex < 9; rowIndex++) {
      for (let colIndex = 0; colIndex < 9; colIndex++) {
        ...
        if (valueBoard[(row * 9) + col] == 0 || valueBoard[(row * 9) + col] == undefined) { 
          ...
          break; //once we've found an empty cell, break out of col loop
    ...
    /*copying the "permanent" numbers into solution */
    for (let solutionIndex = 0; solutionIndex < valueBoard.length; solutionIndex++) {
      storedSolution[solutionIndex] = valueBoard[solutionIndex];
    }
      
    const possibleNumArray = this.getPossibleNumberInThisSquares(valueBoard, (row * 9) + col);
    const possibleNumbersSet = new Set();

    for (let possibleNumIndex = 0; possibleNumIndex < possibleNumArray.length();
      possibleNumIndex++) {
      possibleNumbersSet.add(parseInt(possibleNumArray[possibleNumIndex]));
    }
    
    /*back track solving process, probably not the most effcient b/c 
    I couldn't get a Stack to work :/ so I fell back on using arrays*/
    for (let numindex = 1; numindex <= 9; numindex++) {
      if (possibleNumbersSet.has(numindex)) {
        storedSolution[(row * 9) + col] = numindex;

        if (this.solveHelper(9, valueBoard)) {
          return true;
        } else {
          storedSolution[(row * 9) + col] = 0;
        }
      }
    }
